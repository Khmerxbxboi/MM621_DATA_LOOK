...
const METRICS = [
  ...
  { key:"support",    label:"Support",         domain:[1.0,1.5],  val:d=>d.support,      fmt:d=>nf(d.support,1,3) },
...
function drawDonut(hover){
  ...
  for(let i=0;i<SEA_DATA.length;i++){
    ...
    noStroke(); fill(col);
    ringSlice(donut.cx, donut.cy, donut.r, donut.ir, a0, a0+a);
    a0 += a;
  }
  ...
  const ttl = (idx>=0) ? SEA_DATA[idx].country : mLabel().toUpperCase();
...
function drawBarsDesktop(hover){
  ...
  for(let i=0;i<SEA_DATA.length;i++){
    const d=SEA_DATA[i];
    const xCenter = bars.startX + i*bars.spacing;
    const baseH = valueToH(mVal(d));
    const x = xCenter - bars.barW/2;
    const y = bars.baselineY - baseH;

    noStroke();
    if(i===selected || i===hover) fill(...ACCENT); else fill(48); // fully opaque
    rect(x,y,bars.barW,baseH,6);

    fill(i===selected||i===hover ? 255 : 215);
    textSize(12); text(d.country, xCenter, y-10);
  }
}
...
function drawToolbar(){
  const barX = bars.leftMargin + 10, barW = width - bars.leftMargin - 56, barY=height-78, barH=56;
  noStroke(); fill(18,22,29); rect(barX,barY,barW,barH,10);
  stroke(60); noFill(); rect(barX,barY,barW,barH,10);
  ...
  stroke(60); line(divX, barY+10, divX, barY+barH-10);
  ...
  for(const b of metricButtons){
    ...
    noStroke();
    if(active){ fill(34,36,42); rect(b.x,b.y,b.w,b.h,8); stroke(...ACCENT); noFill(); rect(b.x,b.y,b.w,b.h,8); noStroke(); fill(240); }
...
function drawPill(){
  layoutPill();
  noStroke(); fill(34,36,42); rect(pill.bounds.x,pill.bounds.y,pill.bounds.w,pill.bounds.h,999);
  stroke(...ACCENT); noFill(); rect(pill.bounds.x,pill.bounds.y,pill.bounds.w,pill.bounds.h,999);
...
function drawModal(idx){
  ...
  push(); translate(width/2,height/2); translate(-W/2,-H/2);

  noStroke(); fill(14,18,26); rect(0,0,W,H,16);
  stroke(80); noFill(); rect(0,0,W,H,16);

  const cx=W-36, cy=12, cw=24,ch=24;
  noStroke(); fill(34,36,42); rect(cx,cy,cw,ch,6);
  stroke(255); strokeWeight(1.5);
  line(cx+7,cy+7,cx+17,cy+17); line(cx+17,cy+7,cx+7,cy+17);

  const title=`${FLAG[d.country]||"🏳️"}  ${d.country}`;
  textAlign(CENTER,TOP); textStyle(BOLD); fill(...INK.primary); textSize(isMobile?20:22); text(title,W/2,18);
  drawBadge((W-(isMobile?W-48:220))/2, isMobile?48:52, isMobile?W-48:220, 30, `${mLabel()}: ${mFmt(d)}`);

  textStyle(NORMAL); fill(...INK.secondary); textSize(isMobile?12:13); textAlign(LEFT,TOP);
  const blurb = explain(d);
  if(isMobile){
    text(blurb, 18, 90, W-36, 90);
    const y=180;
    metricsKV(18,y+0,"GDP per capita",nf(d.gdp,1,3));
    metricsKV(18,y+24,"Social support",nf(d.support,1,3));
    metricsKV(18,y+48,"Healthy life",nf(d.health,1,3));
    metricsKV(18,y+72,"Freedom",nf(d.freedom,1,3));
    metricsKV(18,y+96,"Generosity",nf(d.generosity,1,3));
    metricsKV(18,y+120,"Anti-corruption",nf(d.corruption,1,3));
  }else{
    text(blurb, 24, 96, W/2-36, 90);
    const rx=W/2+12, ry=96;
    metricsKV(rx,ry+0,"GDP per capita",nf(d.gdp,1,3));
    metricsKV(rx,ry+28,"Social support",nf(d.support,1,3));
    metricsKV(rx,ry+56,"Healthy life",nf(d.health,1,3));
    metricsKV(rx,ry+84,"Freedom",nf(d.freedom,1,3));
    metricsKV(rx,ry+112,"Generosity",nf(d.generosity,1,3));
    metricsKV(rx,ry+140,"Anti-corruption",nf(d.corruption,1,3));
  }

  textAlign(RIGHT,BOTTOM); textSize(11); fill(...INK.muted);
  text(isMobile? "Tap outside, press ESC, or × to close" : "Click background, press ESC, or × to close", W-16, H-12);
  pop();
}
function drawBadge(x,y,w,h,txt){
  noStroke(); fill(34,36,42); rect(x,y,w,h,999);
  stroke(...ACCENT); noFill(); rect(x,y,w,h,999);
  fill(...INK.primary); textAlign(CENTER,CENTER); textSize(12); textStyle(BOLD); text(txt, x+w/2, y+h/2+1);
}
function metricsKV(x,y,label,val){
  textAlign(LEFT,CENTER); textSize(12); fill(225); text(label,x,y);
  ...
  noStroke(); textAlign(RIGHT,CENTER); textStyle(BOLD); fill(...INK.primary); text(val,dotsEnd,y);
}
function explain(d){
  const arr=[
  ...
  const trust = d.corruption>=0.40 ? "with strong institutional trust"
             : d.corruption>=0.20 ? "with moderate institutional trust"
             : "where institutional trust is reported lower";
...
function drawAbout(){
  ...
  push(); translate(width/2,height/2); translate(-W/2,-H/2);
  noStroke(); fill(16,20,28); rect(0,0,W,H,16);
  stroke(80); noFill(); rect(0,0,W,H,16);

  textAlign(LEFT,TOP); textStyle(BOLD); fill(...INK.primary);
  textSize(isMobile?18:20);
  text("About: How This Dashboard Defines Happiness", 20, 18);

  textStyle(NORMAL); textSize(isMobile?12:13); fill(...INK.secondary);
  ...
  drawSection(W, y, "What is measured?",
    "People are asked: “On a ladder from 0 (worst possible life) to 10 (best possible life), which step are you on today?” Each country’s average becomes its Happiness Score."); y+=86;
  drawSection(W, y, "Why these factors?",
    "Six drivers correlate strongly with how people rate their lives:"); y+=64;

  const bullets = [
    ["GDP per capita", "Material prosperity and ability to meet needs."],
    ["Social support", "Having someone to rely on boosts resilience."],
    ["Healthy life expectancy", "More healthy years → higher reported well-being."],
    ["Freedom to make life choices", "Autonomy to shape one’s life."],
    ["Generosity", "Giving/volunteering builds social connection."],
    ["Anti-Corruption", "World Happiness Report ‘perceptions of corruption’ factor."]
  ];
  const bx = 28, bw = W-48;
  textAlign(LEFT,TOP); textSize(isMobile?12:13);
  for(const [k,v] of bullets){
    noStroke(); fill(...ACCENT); circle(bx, y+8, 6);
    fill(...INK.primary); textStyle(BOLD); text(k, bx+12, y);
    textStyle(NORMAL); fill(...INK.secondary); text(v, bx+12, y+18, bw);
    y += 48;
    if (y > H-92) break;
  }
...
function drawSection(W, y, title, body){
  textAlign(LEFT,TOP);
  textStyle(BOLD); fill(...INK.primary); textSize(isMobile?14:15);
  text(title, 20, y);
  textStyle(NORMAL); fill(...INK.secondary); textSize(isMobile?12:13);
...
function toggleSelect(i){ selected = (selected===i)? -1 : i; }
...
function barHitDesktop(mx,my){
  ...
  for(let i=0;i<SEA_DATA.length;i++){
    const xCenter = bars.startX + i*bars.spacing;
    const baseH = valueToH(mVal(SEA_DATA[i]));
    const x=xCenter-bars.barW/2, y=bars.baselineY-baseH;
